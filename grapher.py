...
def old_grapher():
    '''First attempt at a graphing program. It sucks.'''
    min_x, min_y = -10, -10
    max_x, max_y = 10, 10
    screen = Screen()
    ...
    hideturtle()
    width(2)
    #draws x and y axes as lines straight through the middle of the screen
    #(not relative values...)
    setpos(-end, 0)
    setpos(0, 0)
    setpos(0, top)
    ...
    x = min_x
    xs = [x]
    for num in range(max_x * 100):
        x = round(x + max_x / 100., 1)
        xs.append(x)
    #print xs
    #xs = [x + (max_x / 100.) for x in range(max_x * 100)
    function = raw_input("Please input your function: ")
    ys = eval("[" + function + " for x in xs]")
    real_xs = [-end * x / max_x for x in xs]
    real_ys = [top * y / max_y for y in ys]
    #print real_ys
    penup()
    #setpos(xs[0], ys[0])
    setpos(real_xs[0], real_ys[0])
    pendown()
    for num in range(len(real_ys)):
        setpos(real_xs[num], real_ys[num])
    screen.listen()
    screen.exitonclick()
...
def axis_draw(min_x, max_x, min_y, max_y):
   ...
   #for each interval tick...
        ...
        backward(20) 
        forward(10)
        #draw a 20 pixel-long tick (10 on each side of axis)
        color('black')
...
(or anything that's not 'True') otherwise: "))
    except: riemannDraw = False
...
(See the Python Turtle Graphics Window) \n"
   ...
   #for each x/y value index...
        #color(random_color()) #makes the function segment a different color
...
def riemann(x_values, y_values, x_axis, y_axis, size):
    '''Draws the integral rectangles based on
        the given x and y values, and the location
