...
noob = canvas.create_line(xcor(), ycor(), 0, 0)
canvas.itemconfigure(noob, fill='red', width=3)
def forwards():
...
def ranger():
    angle = heading() + range_angle
    wall = closechecker()
    displace = wall[0] * math.tan(angle)
    point = (wall[1][0], wall[1][1])
    good = canvas.create_line(position()[0], -position()[1], point[0], -point[1])
    forward(0)
    canvas.delete(good)
...
def rangechecker():
    #heading()
    for line in our_map:
        point = edgechecker(line)
        if point[1] == ycor() and \
           point[0] > xcor():
            canvas.coords(noob, xcor(), -ycor(), point[0], -point[1])
